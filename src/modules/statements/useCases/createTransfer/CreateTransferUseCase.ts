...
class CreateTransferUseCase {
  ...
  async execute({
    ...
    if (amount <= 0) {
      throw new AppError("Amount needs to be greater than 0");
    }

    const userToReceive = await this.usersRepository.findById(receiveUserId);
    ...
    await createStatementUseCase.execute({
      ...
      description,
      type: OperationType.TRANSFER,
      user_id: userToReceive.id as string,
    });
  }
}
